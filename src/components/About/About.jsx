...
function About() {
  return (
    ...
    <div className="w-screen h-screen flex text-white flex-col justify-center items-center text-[22px] font-navfont -z-10 p-2.5 overflow-y-hidden max-md:w-screen max-md:h-screen max-md:bg-gradient-to-t max-md:from-[#3D3D3D] max-md:to-[#1F1F1F] max-lg:max-md:bg-gradient-to-t max-lg:max-md:from-[#3D3D3D] max-lg:max-md:to-[#1F1F1F]">
      <svg className='absolute h-[97vh] w-[99vw] -z-10 mt-[25px] block overflow-hidden max-md:hidden max-lg:max-md:hidden' width="100vw" overflow="hidden" height="100vh" viewBox='0 0 1903 1058' fill="none" preserveAspectRatio='none' xmlns="http://www.w3.org/2000/svg">
      ...
      <motion.h2
        ...
        className='mb-5 text-[#ffbc00] text-4xl font-semibold font-navfont max-md:text-[30px] max-md:pt-5'>About</motion.h2>
      <motion.p
        initial={{ opacity: 0, scale: 0.9 }}
        whileInView={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1 }}
        className='font-[Futura] font-normal justify-around w-1/2 text-white max-md:text-base max-md:w-full max-md:p-2.5'>
